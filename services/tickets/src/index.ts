...
app.get('/health', (_req, res) => {
  res.json({ status: 'ok', service: 'tickets', timestamp: new Date().toISOString() });
});
...
async function bootstrap(): Promise<void> {
  console.info('[tickets] Starting FlowDesk Tickets Service...');
  ...
  app.listen(config.PORT, () => {
    console.info(`[tickets] Tickets service listening on port ${config.PORT} (${config.NODE_ENV})`);
  });
}

process.on('SIGTERM', async () => {
  console.info('[tickets] SIGTERM received. Shutting down gracefully...');
...
bootstrap().catch((err) => {
  console.error('[tickets] Bootstrap failed:', err);
  process.exit(1);
});
